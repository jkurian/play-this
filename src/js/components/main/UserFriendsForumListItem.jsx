  ...
  render() {
    const onClickRequest = (ev, id) => {
      ...
      for (let i=0; i < this.props.userFriendsForums.length; i++) {
          ...
          allFriendsForums.push(<ListItem id={this.props.userFriendsForums[i].id} key={i} secondaryText={this.props.userFriendsForums[i].title} leftIcon={<HearingIcon />} onClick={(ev) => onClickRequest(ev, this.props.userFriendsForums[i].id)}/>)
  }
    return (
      <div>
        {allFriendsForums}
